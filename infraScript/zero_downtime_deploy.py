...
class ServiceManager:
    ...
    def __init__(self, socat_port: int = 9020, sleep_duration: int = 3) -> None:
        ...
        self.services: Dict[str, int] = {
            'memberService_1': 9021,
            'memberService_2': 9022
        }
    ...
    def _find_current_service(self) -> None:
        ...
        if not current_service:
            self.current_name, self.current_port = 'memberService_2', self.services['memberService_2']
        else:
    ...
    def _cleanup_dangling_images(self) -> None:
        ...
        # 먼저 이미지 목록 확인
        cmd_list = "docker images | grep '.*/chibbotec/memberService' | grep '<none>'"
        ...
        # 이미지 정리
        cmd_remove = "docker images | grep '.*/chibbotec/memberService' | grep '<none>' | awk '{print $3}' | xargs -r docker rmi"
    ...
    def _run_container(self, name: str, port: int) -> None:
        os.system(
            f"docker run -d --name={name} --restart unless-stopped -p {port}:9020 -e TZ=Asia/Seoul -v /dockerProjects/chibbotec/memberService/volumes/gen:/gen --pull always ghcr.io/chibbotec/memberService")
    ##
